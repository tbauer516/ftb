...
m.getPeripherals = function(self)
	for _, name in ipairs(peripheral.getNames()) do
		if
			...
			and name ~= "back"
		then
			for _, pType in ipairs({ peripheral.getType(name) }) do
...
m.assignChests = function(self)
	local listOfNames = {}
	for _, chest in ipairs(self.storageArray) do
...
m.getInventoryList = function(self)
	...
	for name, spotArray in pairs(self.inventory) do
		...
		for _, location in ipairs(spotArray) do
...
m.consolidateInventory = function(self)
	...
	for j = 1, #inventoryList do
		...
		for k = 1, #self.inventory[itemName] do
			...
			if
				itemInTheWay ~= nil
				and (self.storageArray[chestI] ~= itemToSort.chest or itemToSort.slot ~= nextSortedIndex)
			then
				...
				for l = 1, #self.inventory[itemInTheWay.name] do
					if
						self.inventory[itemInTheWay.name][l].chest == self.storageArray[chestI]
						and self.inventory[itemInTheWay.name][l].slot == nextSortedIndex
					then
				...
				self.storageArray[chestI].pushItems(
					peripheral.getName(empty.chest),
					nextSortedIndex,
					itemInTheWay.count,
					empty.slot
				)
			...
			if self.storageArray[chestI] ~= itemToSort.chest or itemToSort.slot ~= nextSortedIndex then
				self.storageArray[chestI].pullItems(peripheral.getName(itemToSort.chest), itemToSort.slot)
			end
...
hbar1:setUpdate(function()
	local used = 0
	for _, chest in ipairs(inventoryManager.storageArray) do
...
inventoryManager.refreshList = function(_)
	inventoryManager:mapInventory()
	list1:setLines(inventoryManager:getInventoryList())
	ui:display(list1)
end
...
if args[1] == "run" then
	...
	local total = 0
	for i, _ in ipairs(inventoryManager.storageArray) do
