...
m._click = function(...)
	print(select("#", ...) .. " number of args passed")
	error("need to 'setClick' for the scrolllist")
end
...
m.click = function(self, win, x, y)
	...
	local scrollIndex = self._scrollpos + y - winY
	local scrollItem = self._lines[scrollIndex]
	if scrollItem ~= nil then
		self._click(scrollItem.value)
	end
end
